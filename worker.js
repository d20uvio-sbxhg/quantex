...
export default {
  async fetch(request) {
    ...
    try {
      ...
      // 8. 健康檢查
      if (path === '/health') {
        return jsonResponse({ status: 'ok', version: 'v1.4', time: new Date().toISOString() });
      }
...
async function proxyYahoo(params) {
  ...
  const results = await Promise.all(symList.map(async (sym) => {
    try {
      ...
      // 嘗試從 v10 取得基本面數據
      let roe = null, revenueGrowth = null, earningsGrowth = null, trailingPE = null;
      try {
        const v10url = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(sym)}?modules=financialData,defaultKeyStatistics`;
        const v10res = await fetch(v10url, { headers: { 'User-Agent': 'Mozilla/5.0' }, cf: { cacheTtl: 3600 } });
        if (v10res.ok) {
          ...
          if (fd) {
            ...
            trailingPE = ks?.trailingEps?.raw ? price / ks.trailingEps.raw : null;
          }
        }
      } catch(e) {}
