...
export default {
  async fetch(request, env) {
    ...
    if (path === '/dividend/refresh') {
      ...
      return jsonResponse({ ok: true, refreshed: true, poolTried: DEFAULT_TW_STOCKS.length, rawTotal: res.rawTotal, stockCount: res.stockCount, tokenSet: !!(env && env.FINMIND_TOKEN) });
    }
...
async function proxyFinMindDividend(env, years, maxStocks) {
  ...
  for (let pi = 0; pi < pool.length; pi++) {
    ...
    try {
      ...
      if (!resp.ok) { errStocks++; if (resp.status === 402 || resp.status === 429) { console.log('[dividend] rate/quota at', sym, resp.status); break; } continue; }
      const j = await resp.json();
      const rows = (j && j.data) || [];
