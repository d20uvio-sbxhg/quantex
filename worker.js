...
async function proxyUniverseReview(env, params) {
  ...
  // 上市 TWSE
  try {
    const r = await fetch('https://www.twse.com.tw/exchangeReport/STOCK_DAY_ALL?response=json', { headers: { 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://www.twse.com.tw' }, cf: { cacheTtl: 3600 } });
    const j = await r.json();
    const fields = j.fields || [];
    let iAmt = fields.findIndex(f => typeof f === 'string' && (f.indexOf('成交金額') >= 0 || f.indexOf('成交值') >= 0));
    if (iAmt < 0) iAmt = 3;
    (j.data || []).forEach(row => { const sym = String(row[0] || '').trim(); if (/^[0-9]{4}$/.test(sym)) { liq[sym] = num(row[iAmt]); names[sym] = String(row[1] || '').trim(); } });
  } catch (e) { srcErr.push('twse:' + (e && e.message || e)); }
  // 上櫃 TPEx OpenAPI
  try {
  ...
  } catch (e) { srcErr.push('tpex:' + (e && e.message || e)); }
