...
export default {
  async fetch(request, env) {
    ...
    if (path === '/dividend/refresh') {
      // v2.38: 分批增量 — 每次抓一批(預設25檔),併進既有 KV,可重複呼叫累積到滿(避免超時)
      const BATCH = parseInt(params.get('n') || '25', 10);
    ...
    try {
      ...
      try {
        ...
        if (stillFilling || weekDue) {
          ...
          const resC = await proxyFinMindDividend(env, 5, totalC, offC, 25);
          ...
          const nextC = offC + 25, doneC = nextC >= totalC;
