...
export default {
  ...
  async scheduled(event, env, ctx) {
    try {
      ...
      // v2.45: 法人買賣超歷史分批填充(第三套測試用)。等營收填完才填,錯開抓取
      try {
        const revSnap2 = await env.QUANTEX_KV.get('revsnap_v1', { type: 'json' });
        const revDone2 = !(!revSnap2 || (revSnap2._nextOffset && revSnap2._nextOffset > 0));
        ...
        if (revDone2 && (instFilling || instWeekDue)) {
      ...
      // v2.47: 歷史月收盤分批填充(跨股壓測用)。等法人填完才填,鏈式錯開
      try {
        const instSnap2 = await env.QUANTEX_KV.get('instmonthly_v1', { type: 'json' });
        const instDone2 = !(!instSnap2 || (instSnap2._nextOffset && instSnap2._nextOffset > 0));
        ...
        if (instDone2 && (phFilling || phWeekDue)) {
...
async function proxyFinMindRevenue(env, offset, limit) {
  ...
  const startDate = (nowD.getFullYear() - 4) + '-01-01';  // v2.46: 3年→5年,涵蓋 2022 空頭供策略壓測
  const token = (env && env.FINMIND_TOKEN) ? env.FINMIND_TOKEN : '';
  const headers = token ? { 'Authorization': 'Bearer ' + token } : {};
  const byStock = {};
...
async function proxyFinMindPriceHist(env, offset, limit) {
  // v2.47: 逐檔抓 5 年日收盤 TaiwanStockPrice,聚合成「月底收盤」供跨股策略壓測(尤其 2022 空頭)
  const nowD = new Date(Date.now() + 8 * 3600 * 1000);
