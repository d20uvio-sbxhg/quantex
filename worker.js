    ...
    try {
      ...
      if (path === '/twse/financials') {
        try {
          ...
          const res = await fetch(url, {
            headers: { 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://www.twse.com.tw' },
            cf: { cacheTtl: 86400 } // 24小時快取（季報不常更新）
          });
          const data = await res.json();
          const result = {};
          // 欄位: [股票代號, 股票名稱, 股本(億), 每股淨值, EPS(元), 股東權益報酬率(%), 負債比率(%), 流動比率(%)]
          if (data.data) {
            data.data.forEach(row => {
              const symbol = row[0].trim();
              const roe = parseFloat(row[5]);
              const eps = parseFloat(row[4]);
              const pb  = parseFloat(row[3]); // 每股淨值
              result[symbol] = {
                roe: isNaN(roe) ? null : roe / 100,  // 轉成小數（0.15 = 15%）
      ...
      // 13. 健康檢查
      if (path === '/health') {
        return jsonResponse({ status: 'ok', version: 'v1.8', time: new Date().toISOString() });
      }
...
async function proxyYahoo(params) {
  ...
  // 用 Yahoo v7 quote API 批次抓報價（極快，不含歷史）
  try {
...
async function proxyYahooHistory(params) {
  ...
  try {
    ...
    for (let i = 0; i < times.length; i++) {
      if (closes[i]) {
        hist.push({ d: new Date(times[i]*1000).toLocaleDateString('zh-TW',{month:'numeric',day:'numeric'}), c: closes[i], v: volumes[i]||0 });
      }
    }
