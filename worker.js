  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/holders/refresh') {
      ...
      try {
        ...
        return jsonResponse({ ok: true, batchOffset: off, batchGot: res.okStocks, sampleLevels: res.sampleLevels, totalStocks: Object.keys(merged).length, nextOffset: done ? 0 : next, done: done, poolTotal: total, tokenSet: !!(env && env.FINMIND_TOKEN), hint: done ? '大戶籌碼全部抓完' : ('還沒完,再開 /holders/refresh?offset=' + next) });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    if (path === '/cloud/picks') {
...
async function proxyFinMindHolders(env, offset, limit) {
  ...
  return { ok: true, updatedAt: Date.now(), okStocks: okStocks, stockCount: Object.keys(byStock).length, sampleLevels: Object.keys(levelSet), byStock: byStock };
}
