...
export default {
  async fetch(request, env) {
    ...
    if (path === '/dividend/refresh') {
      // 手動觸發抓取(部署後驗證用:看 rawTotal / stockCount)
      const res = await proxyFinMindDividend(env, 5);
      try { await env.QUANTEX_KV.put('dividend_snapshot_v1', JSON.stringify(res)); } catch (e) { return jsonResponse({ ok: false, error: 'kv_put_failed: ' + e.message, rawTotal: res.rawTotal, stockCount: res.stockCount }); }
      return jsonResponse({ ok: true, refreshed: true, poolTried: DEFAULT_TW_STOCKS.length, rawTotal: res.rawTotal, stockCount: res.stockCount, priceOk: res.priceOk, tokenSet: !!(env && env.FINMIND_TOKEN) });
    }
  ...
  async scheduled(event, env, ctx) {
    try {
        ...
        try {
          ...
          if (lastYm !== ymNow) {
            ...
            if (j && j.data && Object.keys(j.data).length > 100) {
              const slim = {};
              Object.keys(j.data).forEach(function(sym){ const r = j.data[sym]; slim[sym] = [r.pe, r.pb, r.yield, r.close]; });
      ...
      // v2.33: 每週股利快照 — 台灣週一 10:00(UTC 2:00),抓近5年股利政策表存 KV
      if (utcHour === 2 && utcMin < 10) {
        try {
          const twNow2 = new Date(Date.now() + 8 * 3600 * 1000);
          if (twNow2.getUTCDay() === 1) {
            const dweek = twNow2.toISOString().slice(0, 10).replace(/-/g, '');
            const lastDiv = await env.QUANTEX_KV.get('dividend_lastweek_v1');
            if (lastDiv !== dweek) {
              const res = await proxyFinMindDividend(env, 5);
              if (res && res.stockCount > 50) {
                await env.QUANTEX_KV.put('dividend_snapshot_v1', JSON.stringify(res));
                await env.QUANTEX_KV.put('dividend_lastweek_v1', dweek, { expirationTtl: 30 * 86400 });
                console.log('[cron-dividend] saved', dweek, 'stocks', res.stockCount, 'raw', res.rawTotal);
              } else {
                console.log('[cron-dividend] too few stocks, skip', res ? res.stockCount : 'null');
              }
            }
          }
...
async function proxyFinMindDividend(env, years, maxStocks) {
  ...
  const pool = DEFAULT_TW_STOCKS.slice(0, maxStocks);
  let rateHits = 0;
  ...
  for (let qi = 0; qi < dsyms.length; qi++) {
    ...
    // v2.37: 順便抓 PER/PBR(FinMind TaiwanStockPER 免費)
    try {
      const perurl = 'https://api.finmindtrade.com/api/v4/data?dataset=TaiwanStockPER&data_id=' + encodeURIComponent(sym) + '&start_date=' + priceStart;
      const perresp = await fetch(perurl, { headers: headers });
      if (perresp.ok) {
        const perj = await perresp.json();
        const perrows = (perj && perj.data) || [];
        if (perrows.length) {
          const lp = perrows[perrows.length - 1];
          const pe = parseFloat(lp.PER) || 0, pb = parseFloat(lp.PBR) || 0;
          if (pe > 0) byStock[sym].pe = Math.round(pe * 10) / 10;
          if (pb > 0) byStock[sym].pb = Math.round(pb * 100) / 100;
        }
      } else if (perresp.status === 402 || perresp.status === 429) { await new Promise(r => setTimeout(r, 1200)); }
    } catch (e) {}
  }
