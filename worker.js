...
export default {
  async fetch(request, env) {
    ...
    if (path === '/dividend/refresh') {
      ...
      return jsonResponse({ ok: true, refreshed: true, years: res.years, rawTotal: res.rawTotal, stockCount: res.stockCount, tokenSet: !!(env && env.FINMIND_TOKEN) });
    }
...
async function proxyFinMindDividend(env, years) {
  years = years || 5;
  ...
  let rawTotal = 0;
  const yearsFetched = [];
  for (let y = nowY - (years - 1); y <= nowY; y++) {
    const url = 'https://api.finmindtrade.com/api/v4/data?dataset=TaiwanStockDividend'
      + '&start_date=' + y + '-01-01&end_date=' + y + '-12-31';
    try {
      const resp = await fetch(url, { headers: headers });
      if (!resp.ok) { console.log('[dividend] year', y, 'http', resp.status); continue; }
      const j = await resp.json();
      const rows = (j && j.data) || [];
      rawTotal += rows.length;
      yearsFetched.push(y);
      rows.forEach(r => {
        const sym = r.stock_id; if (!sym) return;
        ...
        byStock[sym].divs.push({ year: r.year || ('' + y), cash: cash, stock: stock, cashEx: cashEx, stockEx: stockEx, base: r.date || '' });
        byStock[sym].cash5sum += cash;
        byStock[sym].stock5sum += stock;
      });
    } catch (e) { console.log('[dividend] year', y, 'err', e.message); }
  }
  // 每檔算平均 + 找最近一個「未來除息日」
  ...
  return { ok: true, updatedAt: Date.now(), years: yearsFetched, rawTotal: rawTotal, stockCount: Object.keys(byStock).length, byStock: byStock };
}
