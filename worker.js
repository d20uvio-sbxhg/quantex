...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/names') {
      ...
      if (params.get('refresh') === '1' || !snap || !snap.names || Object.keys(snap.names).length < 100 || !snap.industries) {
    ...
    if (path === '/sync/get') {
      // v2.69: 跨裝置同步 — 讀共享狀態(名單基準/WFV結果)
      try {
        const key = String(params.get('key') || '');
        if (!/^(listbase|wfv)_(tw|us)$/.test(key)) return jsonResponse({ ok: false, error: 'bad key' });
        const v = await env.QUANTEX_KV.get('sync_' + key, { type: 'json' });
        return jsonResponse({ ok: true, key: key, value: v || null });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    if (path === '/sync/set') {
      // v2.69: 寫共享狀態。listbase 伺服器端 20h 輪替(全裝置同一份基準);wfv 較新才覆蓋
      try {
        const key = String(params.get('key') || '');
        if (!/^(listbase|wfv)_(tw|us)$/.test(key)) return jsonResponse({ ok: false, error: 'bad key' });
        const raw = params.get('data') || '';
        if (!raw || raw.length > 6000) return jsonResponse({ ok: false, error: 'bad size' });
        let obj = null; try { obj = JSON.parse(raw); } catch (e) { return jsonResponse({ ok: false, error: 'bad json' }); }
        if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return jsonResponse({ ok: false, error: 'bad payload' });
        const cur = await env.QUANTEX_KV.get('sync_' + key, { type: 'json' });
        const now = Date.now();
        if (key.indexOf('listbase_') === 0) {
          if (cur && cur.ts && (now - cur.ts) < 20 * 3600000) return jsonResponse({ ok: true, stored: false, value: cur });
          obj.ts = now;
          await env.QUANTEX_KV.put('sync_' + key, JSON.stringify(obj));
          return jsonResponse({ ok: true, stored: true, value: obj });
        }
        if (cur && cur.ts && obj.ts && obj.ts <= cur.ts) return jsonResponse({ ok: true, stored: false, value: cur });
        obj.ts = obj.ts || now;
        await env.QUANTEX_KV.put('sync_' + key, JSON.stringify(obj));
        return jsonResponse({ ok: true, stored: true, value: obj });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    ...
    if (path === '/status') {
      // v2.68: 系統健康 — 各資料源上次更新時間 + cron 狀態
      try {
        const keys = ['dailysnap_v1','chipsdaily_v1','revsnap_v1','pricehist_v1','finsnap_v1','instsnap_v1','namesnap_v1'];
        const labels = { dailysnap_v1:'日線價量', chipsdaily_v1:'籌碼(外資/融資券)', revsnap_v1:'月營收', pricehist_v1:'股價歷史', finsnap_v1:'財報', instsnap_v1:'法人', namesnap_v1:'股名/產業' };
        const snaps = [];
        for (let i = 0; i < keys.length; i++) {
          const k = keys[i];
          let v = null; try { v = await env.QUANTEX_KV.get(k, { type: 'json' }); } catch (e) {}
          const cnt = v ? (v.byStock ? Object.keys(v.byStock).length : (v.count != null ? v.count : (v.names ? Object.keys(v.names).length : null))) : null;
          snaps.push({ key: k, label: labels[k] || k, updatedAt: v ? (v.updatedAt || v.ts || v.computedAt || null) : null, count: cnt, ready: !!v });
        }
        let hb = null, cron = null;
        try { hb = await env.QUANTEX_KV.get('cron_heartbeat_v1', { type: 'json' }); } catch (e) {}
        try { cron = await env.QUANTEX_KV.get('finmind_cron_v1', { type: 'json' }); } catch (e) {}
        return jsonResponse({ ok: true, now: Date.now(), heartbeat: hb, finmindCron: cron, snapshots: snaps });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    if (path === '/universe/review') {
      // v2.66: 母體體檢 — 全台股成交值排名 vs 現有母體,給增刪建議
      try { return await proxyUniverseReview(env, params); }
      catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    if (path === '/chips/ratios') {
      // v2.62: 輕量券資比端點(給 App 軋空旗標)。回 { sym: 券資比(融券餘額/融資餘額) }
      try {
        const snap = await env.QUANTEX_KV.get('chipsdaily_v1', { type: 'json' });
        const out = {};
        if (snap && snap.byStock) {
          Object.keys(snap.byStock).forEach(function (sym) {
            const rec = snap.byStock[sym]; const mbD = rec.mb || {}, sbD = rec.sb || {};
            const mbK = Object.keys(mbD).sort(); const sbK = Object.keys(sbD).sort();
            if (mbK.length && sbK.length) {
              const lastD = mbK[mbK.length - 1];
              const mb = mbD[lastD], sb = (sbD[lastD] != null ? sbD[lastD] : sbD[sbK[sbK.length - 1]]);
              if (mb && mb > 0 && sb != null) out[sym] = Math.round(sb / mb * 10000) / 10000;
            }
          });
        }
        return jsonResponse({ ok: true, updatedAt: (snap && snap.updatedAt) || 0, count: Object.keys(out).length, ratios: out });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    ...
    if (path === '/daily/autofill') {
      // v2.63: 自動連續 — 記住進度,重複按「同一個網址」即可(不用改 offset)。?reset=1 從頭
      try {
        const cur = await env.QUANTEX_KV.get('dailysnap_v1', { type: 'json' });
        const total = DEFAULT_TW_STOCKS.length;
        const off = (params.get('reset') || !cur || cur._nextOffset == null) ? 0 : cur._nextOffset;
        if (off >= total) return jsonResponse({ ok: true, progress: total + '/' + total + ' 檔', done: true, hint: '\u2705 \u5df2\u5b8c\u6210!\u958b /daily/snapshot?download=1 \u4e0b\u8f09' });
        const res = await proxyFinMindDaily(env, off, 24);
        const merged = (off > 0 && cur && cur.byStock) ? cur.byStock : {};
        Object.keys(res.byStock).forEach(k => { merged[k] = res.byStock[k]; });
        const mergedVP = (off > 0 && cur && cur.volProfile) ? cur.volProfile : {};
        Object.keys(res.volProfile || {}).forEach(k => { mergedVP[k] = res.volProfile[k]; });
        const next = off + 24, done = next >= total;
        await env.QUANTEX_KV.put('dailysnap_v1', JSON.stringify({ ok: true, updatedAt: Date.now(), byStock: merged, volProfile: mergedVP, stockCount: Object.keys(merged).length, _nextOffset: done ? total : next }));
        return jsonResponse({ ok: true, progress: Math.min(next, total) + '/' + total + ' \u6a94', batchGot: res.okStocks, totalStocks: Object.keys(merged).length, done: done, hint: done ? '\u2705 \u65e5\u7dda\u5168\u90e8\u6293\u5b8c!\u4e0b\u4e00\u6b65:\u958b /daily/snapshot?download=1 \u4e0b\u8f09' : '\ud83d\udd04 \u9084\u6c92\u5b8c \u2014 \u518d\u6309\u4e00\u6b21\u540c\u4e00\u500b\u7db2\u5740(\u4e0d\u7528\u6539)' });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    if (path === '/chips/autofill') {
      // v2.63: 自動連續 — 重複按「同一個網址」即可。?reset=1 從頭
      try {
        const cur = await env.QUANTEX_KV.get('chipsdaily_v1', { type: 'json' });
        const total = DEFAULT_TW_STOCKS.length;
        const off = (params.get('reset') || !cur || cur._nextOffset == null) ? 0 : cur._nextOffset;
        if (off >= total) return jsonResponse({ ok: true, progress: total + '/' + total + ' 檔', done: true, hint: '\u2705 \u5df2\u5b8c\u6210!\u958b /chips/snapshot?download=1 \u4e0b\u8f09' });
        const res = await proxyFinMindChips(env, off, 12);
        const merged = (off > 0 && cur && cur.byStock) ? cur.byStock : {};
        Object.keys(res.byStock).forEach(k => { merged[k] = res.byStock[k]; });
        const next = off + 12, done = next >= total;
        await env.QUANTEX_KV.put('chipsdaily_v1', JSON.stringify({ ok: true, updatedAt: Date.now(), byStock: merged, stockCount: Object.keys(merged).length, _nextOffset: done ? total : next }));
        return jsonResponse({ ok: true, progress: Math.min(next, total) + '/' + total + ' \u6a94', batchGot: res.okStocks, totalStocks: Object.keys(merged).length, done: done, hint: done ? '\u2705 \u7c4c\u78bc\u5168\u90e8\u6293\u5b8c!\u4e0b\u4e00\u6b65:\u958b /chips/snapshot?download=1 \u4e0b\u8f09' : '\ud83d\udd04 \u9084\u6c92\u5b8c \u2014 \u518d\u6309\u4e00\u6b21\u540c\u4e00\u500b\u7db2\u5740(\u4e0d\u7528\u6539)' });
      } catch (e) { return jsonResponse({ ok: false, error: String(e && e.message || e) }); }
    }
    if (path === '/cloud/picks') {
  ...
  async scheduled(event, env, ctx) {
    try {
      ...
      // v2.64: FinMind 日線+籌碼「自動分批更新」— 台灣凌晨2點後(UTC>=18,FinMind已更新、且避開其他cron區塊)每次觸發處理一段,當日填完就閒置到隔天。免手動灌。
      if (utcHour >= 18) {
        try {
          const twDay = new Date(Date.now() + 8 * 3600 * 1000).toISOString().slice(0, 10).replace(/-/g, '');
          let cs = await env.QUANTEX_KV.get('finmind_cron_v1', { type: 'json' });
          if (!cs) cs = { phase: 'daily', dOff: 0, cOff: 0, doneDay: '' };
          if (cs.phase === 'done' && cs.doneDay !== twDay) cs = { phase: 'daily', dOff: 0, cOff: 0, doneDay: cs.doneDay };
          const total = DEFAULT_TW_STOCKS.length;
          if (cs.phase === 'daily') {
            const lim = Math.min(40, total - cs.dOff);
            const cur = await env.QUANTEX_KV.get('dailysnap_v1', { type: 'json' });
            const res = await proxyFinMindDaily(env, cs.dOff, lim);
            const merged = (cs.dOff > 0 && cur && cur.byStock) ? cur.byStock : {};
            Object.keys(res.byStock).forEach(function (k) { merged[k] = res.byStock[k]; });
            const mvp = (cs.dOff > 0 && cur && cur.volProfile) ? cur.volProfile : {};
            Object.keys(res.volProfile || {}).forEach(function (k) { mvp[k] = res.volProfile[k]; });
            cs.dOff += lim;
            await env.QUANTEX_KV.put('dailysnap_v1', JSON.stringify({ ok: true, updatedAt: Date.now(), byStock: merged, volProfile: mvp, stockCount: Object.keys(merged).length, _nextOffset: cs.dOff >= total ? total : cs.dOff }));
            if (cs.dOff >= total) cs.phase = 'chips';
            console.log('[cron-finmind] daily', cs.dOff, '/', total);
          } else if (cs.phase === 'chips') {
            const lim = Math.min(20, total - cs.cOff);
            const cur = await env.QUANTEX_KV.get('chipsdaily_v1', { type: 'json' });
            const res = await proxyFinMindChips(env, cs.cOff, lim);
            const merged = (cs.cOff > 0 && cur && cur.byStock) ? cur.byStock : {};
            Object.keys(res.byStock).forEach(function (k) { merged[k] = res.byStock[k]; });
            cs.cOff += lim;
            await env.QUANTEX_KV.put('chipsdaily_v1', JSON.stringify({ ok: true, updatedAt: Date.now(), byStock: merged, stockCount: Object.keys(merged).length, _nextOffset: cs.cOff >= total ? total : cs.cOff }));
            if (cs.cOff >= total) { cs.phase = 'done'; cs.doneDay = twDay; }
            console.log('[cron-finmind] chips', cs.cOff, '/', total);
          }
          await env.QUANTEX_KV.put('finmind_cron_v1', JSON.stringify(cs));
        } catch (e) { console.error('[cron-finmind] error:', e.message); }
      }
...
async function proxyFinMindStockInfo(env) {
  ...
  try {
    ...
    const names = {};
    const industries = {};
    rows.forEach(r => { if (r.stock_id && r.stock_name && !names[r.stock_id]) names[r.stock_id] = r.stock_name; if (r.stock_id && r.industry_category && !industries[r.stock_id]) industries[r.stock_id] = r.industry_category; });
    return { ok: true, updatedAt: Date.now(), count: Object.keys(names).length, names: names, industries: industries };
...
function volProfileOf(pv) {
  ...
  const top = bv.map(function (v, i) { return [v, i]; }).sort(function (a, b) { return b[0] - a[0]; }).slice(0, 8).map(function (x) { return x[1]; });
...
async function proxyUniverseReview(env, params) {
  // v2.66: 母體自動體檢 — 抓全台股(上市STOCK_DAY_ALL + 上櫃TPEx)成交值排名,算 top N,對照現有母體給增刪清單。
  const N = Math.min(300, Math.max(100, parseInt(params.get('n') || '200', 10) || 200));
  const liq = {}, names = {}, srcErr = [];
  const num = (x) => { const v = parseFloat(String(x == null ? '' : x).replace(/,/g, '')); return isNaN(v) ? 0 : v; };
  // 上市 TWSE
  try {
    const r = await fetch('https://openapi.twse.com.tw/v1/exchangeReport/STOCK_DAY_ALL', { headers: { 'User-Agent': 'Mozilla/5.0', 'accept': 'application/json' }, cf: { cacheTtl: 3600 } });
    const arr = await r.json();
    if (Array.isArray(arr)) { arr.forEach(o => { const sym = String(o.Code || '').trim(); if (/^[0-9]{4}$/.test(sym)) { liq[sym] = num(o.TradeValue); names[sym] = String(o.Name || '').trim(); } }); }
    else { srcErr.push('twse:not_array'); }
  } catch (e) { srcErr.push('twse:' + String(e && e.message || e).slice(0, 60)); }
  // 上櫃 TPEx OpenAPI
  try {
    const r2 = await fetch('https://www.tpex.org.tw/openapi/v1/tpex_mainboard_daily_close_quotes', { headers: { 'User-Agent': 'Mozilla/5.0' }, cf: { cacheTtl: 3600 } });
    const arr = await r2.json();
    if (Array.isArray(arr)) arr.forEach(o => {
      const sym = String(o.SecuritiesCompanyCode || o.Code || o.code || '').trim();
      const amt = num(o.TradeValue || o.TransactionAmount || o.Amount || o.TradingAmount);
      if (/^[0-9]{4}$/.test(sym)) { liq[sym] = amt; const nm = o.CompanyName || o.Name || o.name; if (nm) names[sym] = String(nm).trim(); }
    });
  } catch (e) { srcErr.push('tpex:' + String(e && e.message || e).slice(0, 60)); }
  const ranked = Object.keys(liq).sort((a, b) => liq[b] - liq[a]);
  const topN = ranked.slice(0, N), topSet = new Set(topN);
  const univ = DEFAULT_TW_STOCKS, univSet = new Set(univ);
  const add = topN.filter(x => !univSet.has(x));
  const remove = univ.filter(x => liq[x] != null && !topSet.has(x));   // 只移除有資料且掉出N名的
  const noData = univ.filter(x => liq[x] == null);                     // 抓不到的保留
  return jsonResponse({
    ok: true, computedAt: Date.now(), n: N, universeSize: univ.length, rankedStocks: ranked.length,
    suggestAdd: add.map(x => ({ sym: x, name: names[x] || '', rank: topN.indexOf(x) + 1 })),
    suggestRemove: remove.map(x => ({ sym: x, liqRankOutOf: ranked.length, rank: ranked.indexOf(x) + 1 })),
    noLiquidityData: noData,
    sourceErrors: srcErr,
    hint: srcErr.length ? '有資料源錯誤(見sourceErrors)——把錯誤貼給Claude修。' : '把 suggestAdd 加入、suggestRemove 移除,noLiquidityData 保留。確認後跟Claude說即可更新母體。'
  });
}
